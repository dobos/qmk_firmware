...
// Add functionality to play a custom lightning effect when a specific key is pressed.

enum KEYCODES_USER {
  KC_RAINBOW = SAFE_RANGE
};
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [MAC_BASE] = LAYOUT_iso_83(
         KC_ESC,   KC_BRID,  KC_BRIU,  KC_MCTL,  KC_LPAD,  RGB_VAD,  RGB_VAI,  KC_MPRV,  KC_MPLY,  KC_MNXT,  KC_MUTE,  KC_VOLD,    KC_VOLU,  KC_RAINBOW,             KC_MUTE,
    ...
    [WIN_BASE] = LAYOUT_iso_83(
         KC_ESC,   KC_F1,    KC_F2,    KC_F3,    KC_F4,    KC_F5,    KC_F6,    KC_F7,    KC_F8,    KC_F9,    KC_F10,   KC_F11,     KC_F12,   KC_RAINBOW,             KC_MUTE,
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    static uint8_t rgb_mode_old;

    if (!process_record_keychron(keycode, record)) {
        return false;
    }

    // When the rainbow button is pressed
    switch (keycode) {
        case KC_RAINBOW:
            if (record->event.pressed) {
                rgb_mode_old = rgb_matrix_get_mode();
                rgb_matrix_mode(RGB_MATRIX_RAINBOW_PINWHEELS);
                register_code(KC_PRINT_SCREEN);
            } else {
                unregister_code(KC_PRINT_SCREEN);
                rgb_matrix_mode(rgb_mode_old);
            }
            return true;
        default:
            return true;
